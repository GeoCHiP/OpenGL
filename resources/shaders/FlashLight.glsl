...
void main() {
    ...
    float theta = max(dot(lightDirection, normalize(-u_Light.direction)), 0.0f);
    ...
    vec3 diffuse = u_Light.diffuse * diff * vec3(texture(u_Material.diffuse, v_TexCoords)) * intensity;
    vec3 specular = u_Light.specular * spec * vec3(texture(u_Material.specular, v_TexCoords)) * intensity;
